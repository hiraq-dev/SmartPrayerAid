...
public class CaldroidGridAdapter extends BaseAdapter {
	...
	public void updateToday() {
				...
				//	Toast.makeText(context, CurrentTimeAPI.global1.apitime, Toast.LENGTH_SHORT).show();



		//Toast.makeText(context,today.toString(),Toast.LENGTH_LONG).show();
	}
	...
	protected DateTime getToday() {
		...
		//String s = writeDate.format(today);
	...
	//	Toast.makeText(context,"lat: "+ CurrentTimeAPI.global1.mylocation,Toast.LENGTH_LONG).show();

	//	Toast.makeText(context,"lat2: "+ myGlobal.timezoneID,Toast.LENGTH_LONG).show();


    if (myGlobal.strLat==0.0) {
	GPSTracker mGPS = new GPSTracker(context);
	try {
		if (mGPS.canGetLocation) {
			mGPS.getLocation();
			myGlobal.strLat = mGPS.getLatitude();
			myGlobal.strLon = mGPS.getLongitude();
			//myGlobal.strLat=-82.86;
			//myGlobal.strLon=135;
			String strlat = String.valueOf(myGlobal.strLat);
			String strlng = String.valueOf(myGlobal.strLon);
			new CurrentTimeAPI().execute(strlat, strlng);
			//updateToday();

			//CalendarHelper.convertDateToDateTime(startDate);
			//new HijriTime().execute(CurrentTimeAPI.global1.apitime);
...
{
	myGlobal.timezoneID = TimeZone.getDefault().getID();
}
	//	Toast.makeText(context,CurrentTimeAPI.global1.result2,Toast.LENGTH_LONG).show();
		...
		if (selectedDates != null && selectedDatesMap.containsKey(dateTime)) {
			if (CaldroidFragment.selectedBackgroundDrawable != -1) {
				...
				//cell_layout.setBackgroundColor(resources.getColor(R.color.caldroid_sky_blue));
			}
		...
		if (shouldResetDiabledView && shouldResetSelectedView) {
			// Customize for today
			if (dateTime.equals(getToday())) {
				//Toast.makeText(context, ""+dateTime.getDay(), Toast.LENGTH_SHORT).show();
				cell_layout.setBackgroundResource(R.drawable.blue_border_blue_bg);
			} else {
				cell_layout.setBackgroundResource(R.drawable.cell_bg);
			}
		...
		//String xx=Numbers.toArabicNumbers(HijriCalendarDate.getSimpleDate(Qurancal, -1));
		//String yy=Numbers.toArabicNumbers(HijriCalendarDate.getSimpleDateDay(Qurancal, 0));



		//String yy= String.valueOf(hh);
	/*	new HijriTime().execute(CurrentTimeAPI.global1.apitime);


		if (HijriTime.global4.myresult!=null ) {

			String hh = HijriCalendarDate.writeIslamicDate().toString();
			int xx = Integer.parseInt(hh)- Integer.parseInt(HijriTime.global4.myresult);
			if (xx==0)
				myGlobal.value=-1;
			myGlobal.myflag=true;

			int[] xd=HijriCalendarDate.ummalQuraCalendar(Qurancal,-1);
			cellView2.setText("" + Numbers.toArabicNumbers(String.valueOf(xd[1])));

			//cellView2.setText("" + Numbers.toArabicNumbers(HijriCalendarDate.getSimpleDate(Qurancal, myGlobal.value)));

		}
		if (HijriTime.global4.myresult==null)*/
		//String hh = HijriCalendarDate.writeIslamicDate().toString();
		//Toast.makeText(context,HijriCalendarDate.writeIslamicDate(),Toast.LENGTH_LONG).show();
		//String xx= HijriCalendarDate.getSimpleDate(Qurancal, -1);

		//String xx= HijriCalendarDate.getSimpleDateYear(Qurancal,0);
		//Toast.makeText(context,HijriCalendarDate.writeIslamicDate(),Toast.LENGTH_LONG).show();


			cellView2.setText("" + Numbers.toArabicNumbers(HijriCalendarDate.getSimpleDate(Qurancal, 0)));
			setCustomResources(dateTime, cellView2, cellView2);
		//Toast.makeText(context,String.valueOf(HijriCalendarDate.getSimpleDateMonth(Qurancal,0)),Toast.LENGTH_LONG).show();
	...
	//  else {
		  ...
		  //cellView2.setText("" + Numbers.toArabicNumbers(HijriCalendarDate.getSimpleDateDay(Qurancal, 0)));
	  //}


		//Toast.makeText(context,"myresult: "+ HijriTime.global4.myresult,Toast.LENGTH_LONG).show();
